    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _PublicToggleTile(song: widget.song),
...
class _PublicToggleTile extends StatelessWidget {
  final Generation song;

  const _PublicToggleTile({required this.song});

  void _showPublicModal(BuildContext context) {
    Navigator.pop(context);
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
      isScrollControlled: true,
      builder: (ctx) => _PublicToggleModal(song: song),
    );
  }
  ...
  Widget build(BuildContext context) {
    return ListTile(
      leading: Container(
        ...
        height: 40,
        decoration: BoxDecoration(
          color: song.isPublic ? Colors.green.withOpacity(0.1) : Colors.grey.withOpacity(0.1),
          borderRadius: BorderRadius.circular(10),
        ),
        child: Icon(
          song.isPublic ? Icons.public : Icons.public_off,
          color: song.isPublic ? Colors.green : Colors.grey,
          size: 20,
        ),
      ),
      title: Text(song.isPublic ? '🌐 Musik Publik' : '🔒 Musik Private'),
      subtitle: Text(
        song.isPublic ? 'Tampil di Explore' : 'Hanya kamu yang bisa lihat',
        style: TextStyle(color: Colors.grey[500], fontSize: 12),
      ),
      trailing: const Icon(Icons.chevron_right, color: Colors.grey),
      onTap: () => _showPublicModal(context),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
    );
  }
}

class _PublicToggleModal extends StatefulWidget {
  final Generation song;

  const _PublicToggleModal({required this.song});

  @override
  State<_PublicToggleModal> createState() => _PublicToggleModalState();
}

class _PublicToggleModalState extends State<_PublicToggleModal> {
  bool _isLoading = false;

  @override
  Widget build(BuildContext context) {
    final isPublic = widget.song.isPublic;
            ...
            child: Column(
              children: [
                ...
                Container(
                  ...
                  height: 80,
                  decoration: BoxDecoration(
                    color: isPublic ? Colors.green.withOpacity(0.2) : Colors.grey.withOpacity(0.2),
                ...
                const SizedBox(height: 16),
                Text(
                  isPublic ? 'Musik Publik' : 'Musik Private',
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(
                              isPublic ? 'Tampil di Explore' : 'Tersembunyi',
                              style: const TextStyle(fontWeight: FontWeight.bold),
                            ),
                            Text(
                              isPublic
                                  ? 'Semua pengguna bisa mendengarkan musik ini'
                                  : 'Hanya kamu yang bisa melihat dan mendengarkan',
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'Pengguna lain tidak bisa download musik kamu, hanya dengarkan saja',
                ...
                Expanded(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: _isLoading ? null : () async {
                      ...
                      await context.read<MusicProvider>().togglePublic(widget.song.id);
                      if (mounted) {
                        Navigator.pop(context);
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Row(
                              children: [
                                ...
                                const SizedBox(width: 8),
                                Text(isPublic ? 'Musik sekarang Private' : 'Musik sekarang Public!'),
                              ],
                            ),
                            backgroundColor: isPublic ? Colors.grey[700] : Colors.green,
                    ...
                    icon: _isLoading
                        ...
                        : Icon(isPublic ? Icons.lock : Icons.public),
                    label: Text(isPublic ? 'Jadikan Private' : 'Jadikan Public'),
